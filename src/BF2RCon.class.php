...
require dirname(__FILE__).'/BF2RConBase.class.php';

class BF2RCon extends BF2RConBase {
	...
	public function get_reserved_list() {
		if(!($result = $this->query('exec reservedSlots.list')))
			return array();
	...
	public function get_map_list() {
		if(!($result = $this->query('exec mapList.list')))
			return array();
	...
	public function get_banned_address_list() {
		if(!($result = $this->query('exec admin.listBannedAddresses')))
			return array();
		...
		foreach($result as $ban) {
			...
			if(strpos($expires, 'Time Left: ') === 0)
				$expires = substr($expires, 11);
	...
	public function get_banned_key_list() {
		if(!($result = $this->query('exec admin.listBannedKeys')))
			return array();
